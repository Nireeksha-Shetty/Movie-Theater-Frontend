...
import '../foodandBeverages/admin-body.css';
...
let convenienceFeesPercentage = 0.20;
...
const AdminBody = () => {
    ...
    useEffect(()=>{
        ...
        axios.get("http://localhost:2122/all").then(
            (Response)=>
    ...
    const handleSaveNewFoodItemEvent = (e) => {
        ...
        foodItemsList.push(tempFoodItemObj)
         axios.post("http://localhost:2122",tempFoodItemObj).then((Response)=>console.log(Response.data))
    ...
    const editItem = () => {
        ...
        axios.put(`http://localhost:2122/update/item`,tempFoodItemObj).then((response)=>console.log(response.data))
        handleCategorySelectedEvent(currentCategory)
        handleView("edit-popup-bg")
    }
    }
    ...
    return (<>
        <div className="bodymainadmin">
            ...
            <div className="form-popup-admin" id="form-popup" >
                <div className="form-area-admin">
                    <button className="form-exit-btn-admin" onClick={toggleFormPopup}>X</button>
                    <br />
                    <br />
                    <form onSubmit={(e)=>validation(e)}>
                        ...
                        <div className="form-section-admin">
                            <label htmlFor="input-code"><b>Item Code</b></label>
                            ...
                            <label htmlFor="input-code" id="input-code-warning" style={{ color: "red", fontWeight: "light", fontSize: "0.75rem" }}>{itemCodeWarning}</label>
                        </div>
                        <div className="price-div-admin">
                            <div className="form-section-admin">
                        ...
                        <span className="input-errors">{offerPriceError}</span>
                            <label htmlFor="input-quantity" min={0}><b>Quantity</b></label>
                        ...
                        <div className="submit-btn-admin">
                            <button type="submit" className="btn btn-sm btn-primary mt-3"  >Save</button>
                        </div>
            ...
            <div id="delete-popup" className="delete-popup-admin" >
                <div className="delete-warning-admin">
                    <p>This item will be deleted permanently!</p>
                    <div className="delete-choice-admin">
                        <button className="delete-btn" onClick={deleteItem} >
                            delete
                        </button>
                        <button className="cancel-btn-admin" onClick={() => handleView('delete-popup')}>
                            cancel
                        </button>
                    </div>
                </div>
            ...
            <div className="edit-popup-bg-admin" id="edit-popup-bg">
                <div className="edit-popup-area-admin">
                    
                    
                    <label htmlFor="name">Name</label><br />
                    <input placeholder={tempFoodItemDisplayObj.name} defaultValue={tempFoodItemDisplayObj.name} className="name" onInput={e => editAttribute("name", e.target.value)} >
                    </input><br />
                    <label htmlFor="description">Description</label><br />
                    <input placeholder={tempFoodItemDisplayObj.description} defaultValue={tempFoodItemDisplayObj.description} className="description" onInput={e => editAttribute("description", e.target.value)}></input><br />
                    <label htmlFor="category">Category</label><br />
                    <span className="category" onChange={e => editAttribute("category", e.target.value)}>
                    ...
                    <label htmlFor="actual-price">Actual-price</label><br />
                    <input placeholder={tempFoodItemDisplayObj.actualPrice} defaultValue={tempFoodItemDisplayObj.actualPrice} className="actual-price"  onChange={e => editAttribute("actualPrice", e.target.value)} onMouseDown={()=>setActualPriceError("")} ></input><br />
                    <span className="input-errors-admin">{actualPriceError}</span>
                    <label htmlFor="offer-price">Offer-price</label><br />
                    <input placeholder={tempFoodItemDisplayObj.offerPrice} defaultValue={tempFoodItemDisplayObj.offerPrice} className="offer-price" onInput={e => editAttribute("offerPrice", e.target.value)} onMouseDown={()=>setOfferPriceError("")}></input><br />
                    <span className="input-errors-admin">{offerPriceError}</span>
                    <label htmlFor="quantity-available">Quantity-available</label><br />
                    <input placeholder={tempFoodItemDisplayObj.quantityAvailable} defaultValue={tempFoodItemDisplayObj.quantityAvailable} onMouseDown={()=>setQuantityError("")}className="quantity-available" onInput={e => editAttribute("quantityAvailable", e.target.value)} ></input><br />
                    <span className="input-errors-admin">{quantityError}</span>
                    <label htmlFor="Image-url">Image-url</label>
                    <input placeholder={tempFoodItemDisplayObj.itemImageUrl} defaultValue={tempFoodItemDisplayObj.itemImageUrl} className="Image-url" onInput={e => editAttribute("imageUrl", e.target.value)}></input>
                    <div className="buttons-admin">
                        <button className="save-edit-admin btn btn-sm btn-primary mt-3" title="Save edited details" onClick={() => editItem()}>Save</button>
                        <button className="cancel-edit-admin btn btn-sm btn-primary mt-3" title="Cancel editing" onClick={() => handleView("edit-popup-bg")}>Cancel</button>
                    </div>
                </div>
            </div>
            ...
            <div className="body-left-admin">
                {/* ----------------------------Ad-Image display-------------------------- */}
                <div className="adcard-display-admin">
                ...
                <div className="food-category">
                    ...
                    <ul>
                        ...
                        <label htmlFor="select-sorting" style={{fontWeight:600,marginLeft:"90px"}}>Sort By: </label>
                        ...
                        <li className="add-new-food-item-admin">
                            <button
                ...
                <div className="food-item-display-admin">
                    < div className="food-items-admin" >
                        {
                            currentFoodIemsList.length <= 0
                                ...
                                : currentFoodIemsList.map(
                                    item => (
                                        <div className="container-admin" key={item.id} >
                                            <div className="itemImage-admin">
                                            ...
                                            <div className="details-admin">
                                                ...
                                                <div className="food-item-button-container-admin ">
                                                    <div className="actualprice price"><del style={{ color: "red" }}>{item.actualPrice}/-</del></div>
                                                    <div className="offerprice price"><b>{item.offerPrice}/-</b></div>
                                            ...
                                            <div className="discountline-admin"><small>save upto {parseInt(((parseFloat(item.actualPrice) - parseFloat(item.offerPrice)) / parseFloat(item.actualPrice)) * 100)}%</small></div>
                                        </div>
                                    )
                                )
                        }
            ...
            <div className="body-right-admin">
                <div className="body-right-title-admin">
                    <p>ORDER SUMMAARY</p>
                </div>
                <div className="ticket-details-admin">
                    <div className="ticket-price-admin">
                        ...
                        <p className="amount-display-admin">Rs.{totalTicketPrice.toFixed(2)}</p>
                        <p className="theatrename-display-admin">{ticketBookingSummary.theatre}</p>
                    </div>
                    <div className="convenience-fee">
                        <p className="convenience-fee-text">
                            {/* <i className="fa fa-caret-down convenience-fee-icon" aria-hidden="true"></i> */}
                            {" Convenience fees"}
                        </p>
                        <p className="amount-display">Rs.{totalConvenienceFee.toFixed(2)}</p>
                        <p className="sub-convenience-fee conv-fee-text " style={{ marginTop: "-1rem", marginLeft: "2rem" }}>{"Base Amount"}</p>
                        <p className="sub-convenience-fee amount-display" style={{ marginTop: "-1rem" }}>{`Rs. ${baseAmount.toFixed(2)} `}</p>
                        <p className="sub-convenience-fee conv-fee-text " style={{ marginTop: "-1rem", marginLeft: "2rem" }}>{`Integrated GST (IGST) @ ${IGSTPercentage * 100}% `}</p>
                        <p className="sub-convenience-fee amount-display" style={{ marginTop: "-1rem" }}>{`Rs. ${IGSTFee.toFixed(2)} `}</p>
                    </div>
                </div>
                <hr />
                <div className="subtotal-admin">
                    <span className="subtotal-title-admin">{"Subtotal"}</span>
                    <span className="subtotal-amount-admin">Rs.{totalTicketPrice + totalConvenienceFee}</span>
                </div>
                <div className="addons-title">
                    <span>
                        Food and Beverages
                    </span>
                    <span className="addons-title-price">
                        Rs.{totalAddonsCost}
                    </span>
                </div>
                ...
                <div className="addons">
                    <ul className="addons-ul">
                        {Object.keys(addedFoodItems).map(id => (
                            <li key={id}>
                                <button className="button rm-btn" title="remove item from cart" value={id} onClick={(e) => handleFoodItemRemoveEvent(e.target.value)}>
                                    X
                                </button>
                                <span>{foodItemsList.filter(item => item.id == id)[0].name}</span>
                                <button className="button inc-btn" value={id} title="Increase quantity" onClick={(e) => handleFoodItemAddEvent(e.target.value)} >{"+"}</button>
                                <button className="button dec-btn" value={id} title="Decrease quantity" onClick={(e) => handleFoodItemDecrementEvent(e.target.value)}>{"-"}</button>
                                <span className="quantity">{addedFoodItems[id]}{"   "}</span>
                                <span className="item-total-price">Rs.{foodItemsList.filter(item => item.id == id)[0].offerPrice * addedFoodItems[id]}</span>
                            </li>
                        )
                        )}
